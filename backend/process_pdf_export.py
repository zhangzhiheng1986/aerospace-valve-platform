...
def export_dynamic_route_pdf(route: Dict) -> tuple:
    """Export a dynamic (auto-generated) route dict to PDF.

    route must contain: name, material, operations (list of {step,process,method,equipment,time_min}).
    """
    if not route or not route.get('operations'):
        return None, None
    buf = io.BytesIO()
    doc = SimpleDocTemplate(buf, pagesize=A4,
                            leftMargin=15 * mm, rightMargin=15 * mm,
    ...
    story = []
    name = route.get('name', 'Auto-Generated Process Route')
    story.append(Paragraph(f"Work Instruction / 作业指导书: {name}", title_style))
    info = (f"Material: {route.get('material', '-')} | "
            f"Template: {route.get('base_route_id', 'auto')} | "
            f"Valve: {route.get('valve_type', '-')} | "
            f"Steps: {len(route['operations'])} | Total Time: {route.get('total_time_h', 0)} h "
            f"({route.get('total_time_min', 0)} min)")
    story.append(Paragraph(info, body))
    if route.get('size_factor'):
        story.append(Paragraph(
            f"Size factor: {route['size_factor']} (auto-scaled by nominal size)", body))
    story.append(Spacer(1, 6))
    story.append(Paragraph("Operation Sequence / 工序列表", h2))
    rows = [['#', 'Process', 'Method', 'Equipment', 'Time (min)']]
    ...
    story.append(Paragraph(
        "4. 100% leak test at 1.5x working pressure, leak rate < 1E-9 Pa.m3/s", body))
    if route.get('key_points'):
        story.append(Paragraph("Process Notes / 工艺要点", h2))
        for kp in route['key_points']:
            story.append(Paragraph(f"- {kp}", body))
    story.append(Spacer(1, 6))
    story.append(Paragraph(
        "Avis Aerospace Valve R&D Platform / 航天阀门研发平台 | "
        "Auto-Generated " + _now_str(), body))
    doc.build(story)
    buf.seek(0)
    slug = route.get('base_route_id', 'auto')
    filename = f"auto_route_{slug}_{int(time.time())}.pdf"
    return buf, filename
