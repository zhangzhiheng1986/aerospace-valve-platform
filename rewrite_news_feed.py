...
content = '''"""Hang Kong Hang Tian Fa Men Yan Fa Mei Ri Zi Xun Mo Kuai"""
import json
import os
import re
...
def add_news_entry(news_data):
    ...
    index['entries'].insert(0, {
        'date': date_str,
        'title': news_data.get('title', date_str + ' info'),
        'items': news_data.get('items', []),
        'source': news_data.get('source', 'auto'),
    ...
    day_file_name = 'news_' + date_str.replace('-', '') + '.json'
    day_file = os.path.join(NEWS_DIR, day_file_name)
...
def parse_markdown_news(file_path):
    ...
    # Pattern: N. **Title** (Source) - Summary
    # Use \\W to match any non-word char, covers Chinese parens and em-dash
    pat = re.compile(r'(\\d+)\\.\\s+\\*\\*(.+?)\\*\\*\\W+(.+?)\\W{1,3}\\s*(.+)')
    ...
    for line in lines:
        ...
        if m:
            ...
            # Clean up source: remove trailing comma/period
            src = re.sub(r'[,;.]$', '', src)
            items.append({
                'index': idx,
                'title': title,
    ...
    if not items:
        # Fallback: try line-by-line with ** extraction
        for line in lines:
            line = line.strip()
...
with open('backend/news_feed.py', 'w', encoding='utf-8') as f:
    f.write(content)
